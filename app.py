...
def load_model():
    ...
    # https://drive.google.com/file/d/1kJBCHYI-5uGgn2JrZk5zM6V1PcwZd53N/view?usp=sharing ---- Tuned new
    file_id="1kJBCHYI-5uGgn2JrZk5zM6V1PcwZd53N" #Tuned new model
    ...
    output = "high_risk_street_model_tuned_g.joblib"
...
@st.cache_data
def predict_high_risk(city, hour, visibility, weather_condition, precipitation, day_of_week, month, humidity, temperature):
...
day_of_week = st.selectbox("Select Day Of Week", sorted(data['DayOfWeek'].unique()))
hour = st.slider("Select Hour of the Day", 0, 23, 12)
humidity = st.number_input("Humididty (%)", min_value=0, max_value=100, value=25)
temperature = st.number_input("Temperature (F)", min_value=-10, max_value=120, value=50)
...
if st.button("Submit"):
    ...
    with st.spinner("Loading Recommendations, please wait..."):
        result_df = predict_high_risk(city, hour, visibility, weather_condition, precipitation, day_of_week, month, humidity, temperature)

    # Display the results
    st.subheader(f"Streets to Avoid in {city}")
    if not result_df.empty:
        st.write(result_df[['City', 'Street', 'Hour','DayOfWeek', 'Month', 'Temperature(F)','Humidity(%)', 'Weather_Condition', 'Visibility(mi)', 'Precipitation(in)', 'High_Risk_Probability']])
    else:
